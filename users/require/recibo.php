<?php
    ...
    $sql = "SELECT d.nome as disciplina, d.preco_mensal as mensal, d.regime as regime, d.duracao as duracao, count(m.disciplina_id) as pago, m.create_at as inscrido, day(m.create_at) as day, month(m.create_at) as month, year(m.create_at) as year, m.id as codigo, sum(d.preco_mensal) as total FROM disciplina as d INNER JOIN estudantes as e INNER JOIN mensalidades as m WHERE d.id = m.disciplina_id AND m.estudante_id = e.id AND e.id = :id GROUP BY(d.id)";
    $statement = $conn->prepare($sql);
    $statement->bindParam(":id", $estudante, PDO::PARAM_INT);
    $statement->execute();
...
class PDF extends FPDF
{
    ...
    function BasicTable($header, $data){
        ...
        foreach($data as $row)
        {
            ...
            foreach($row as $col){
                if($count == 4){
                	...
                	if($count == 6){
                		if($row->duracao == $row->pago)
                			$this->Cell(28,10,'COMPLETO',0,0,'C');
                		else
                			$this->Cell(28,10, $row->duracao -$row->pago .' EM FALTA',0,0,'C');
                	}
                    if($count == 2)
        ...
        //$total = number_format($total_mensal);
        $this->Cell(162,8,"{$total_mensal} .00MT",0,0,'R');
    }
}
...
$pdf = new PDF();
// Column headings
$header = array('DISCIPLINA','MENSALIDADE','REGIME','DURACAO','PRESTACOES','ESTADO','DATA');
$pdf->AddPage();
...
$pdf->Cell(0,10,'COMPROVATIVO DE PAGAMENTOS DE MENSALIDADES',2,0,'C');
// Line break
$pdf->Ln();
// Set font
...
//header("Location:../inscricao_details.php");
?>